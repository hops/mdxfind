...
static const char gpu_md6256unsalted_str[] =
    ...
    "                pidx /= sz;\n"
    "                /* Byte i in message → A[25 + i/8], LE byte position (i&7)*8 */\n"
    "                int wi = 25 + (i >> 3);\n"
    "                int bi = (i & 7) << 3;\n"
    ...
    "\n"
    "        if (n_app > 0) {\n"
    "            /* Message length stored by host in params or derivable.\n"
    "             * Host stores total message byte length in the low 16 bits\n"
    "             * of the first unused passlen slot. For simplicity, we\n"
    "             * extract it from the control word V = A[24].\n"
    "             * V bits [35:20] = p (padding bits), msglen = 512 - p/8 bytes.\n"
    "             * Actually p is in bits: msglen_bytes = (64*64 - p) / 8 */\n"
    "            ulong V = A[24];\n"
    ...
    "     * Hash = C[12..15] (last 4 words = last 256 bits) */\n"
    "    ulong C12 = A[1749], C13 = A[1750], C14 = A[1751], C15 = A[1752];\n"
    "\n"
    "    /* Byte-swap output to LE for compact table probe */\n"
    "    C12 = bswap64(C12); C13 = bswap64(C13);\n"
    "\n"
    "    /* Split first 2 uint64 into 4 uint32 for probe */\n"
    "    uint hx = (uint)C12, hy = (uint)(C12 >> 32);\n"
    "    uint hz = (uint)C13, hw = (uint)(C13 >> 32);\n"
    "\n"
