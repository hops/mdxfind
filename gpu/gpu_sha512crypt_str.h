...
static const char gpu_sha512crypt_str[] =
    ...
    " */\n"
    "\n"
    "/* ---- SHA512 primitives (big-endian internal) ---- */\n"
    "\n"
    "ulong bswap64_sc(ulong x) {\n"
    ...
    "\n"
    "ulong sc_rotr64(ulong x, uint n) { return (x >> n) | (x << (64 - n)); }\n"
    "\n"
    "void sc_sha512_block(ulong *state, ulong *M) {\n"
    "    ulong W[80];\n"
    "    for (int i = 0; i < 16; i++) W[i] = M[i];\n"
    "    for (int i = 16; i < 80; i++) {\n"
    "        ulong s0 = sc_rotr64(W[i-15], 1) ^ sc_rotr64(W[i-15], 8) ^ (W[i-15] >> 7);\n"
    "        ulong s1 = sc_rotr64(W[i-2], 19) ^ sc_rotr64(W[i-2], 61) ^ (W[i-2] >> 6);\n"
    "        W[i] = W[i-16] + s0 + W[i-7] + s1;\n"
    "    }\n"
    "    ulong a = state[0], b = state[1], c = state[2], d = state[3];\n"
    "    ulong e = state[4], f = state[5], g = state[6], h = state[7];\n"
    "    for (int i = 0; i < 80; i++) {\n"
    "        ulong S1 = sc_rotr64(e, 14) ^ sc_rotr64(e, 18) ^ sc_rotr64(e, 41);\n"
    "        ulong ch = (e & f) ^ (~e & g);\n"
    "        ulong t1 = h + S1 + ch + K512_SC[i] + W[i];\n"
    "        ulong S0 = sc_rotr64(a, 28) ^ sc_rotr64(a, 34) ^ sc_rotr64(a, 39);\n"
