/* gpu_hmac_rmd320.cl -- HMAC-RIPEMD-320 GPU kernels
...
/* RMD_STEP provided by gpu_common.cl.
 * F1-F5 are local short aliases for the RIPEMD boolean functions. */
...
/* HMAC helper: copy global bytes into LE M[] */
void rmd320_copy_bytes(uint *M, int off, __global const uchar *src, int len) {
    uchar *mb = (uchar *)M;
    for (int i = 0; i < len; i++) mb[off + i] = src[i];
}

__kernel void hmac_rmd320_ksalt_batch(
    ...
    __global const uint *overflow_offsets, __global const ushort *overflow_lengths)
{
    ...
    if (klen > 64) {
        uint kstate[10] = { 0x67452301u, 0xEFCDAB89u, 0x98BADCFEu, 0x10325476u, 0xC3D2E1F0u,
                             0x76543210u, 0xFEDCBA98u, 0x89ABCDEFu, 0x01234567u, 0x3C2D1E0Fu };
        uint M[16]; for (int i = 0; i < 16; i++) M[i] = 0;
        rmd320_copy_bytes(M, 0, key, (klen < 64) ? klen : 64);
        if (klen <= 55) { ((uchar *)M)[klen] = 0x80; M[14] = klen * 8; }
        rmd320_block(kstate, M);
        if (klen > 55) {
            for (int i = 0; i < 16; i++) M[i] = 0;
            int rem = klen - 64;
            if (rem > 0) rmd320_copy_bytes(M, 0, key + 64, rem);
            ((uchar *)M)[rem] = 0x80;
            M[14] = klen * 8;
            rmd320_block(kstate, M);
        }
    ...
    uint istate[10] = { 0x67452301u, 0xEFCDAB89u, 0x98BADCFEu, 0x10325476u, 0xC3D2E1F0u,
                         0x76543210u, 0xFEDCBA98u, 0x89ABCDEFu, 0x01234567u, 0x3C2D1E0Fu };
    rmd320_block(istate, M);

    for (int i = 0; i < 16; i++) M[i] = 0;
    if (plen <= 55) {
        rmd320_copy_bytes(M, 0, pass, plen);
        ((uchar *)M)[plen] = 0x80;
        M[14] = (64 + plen) * 8;
        rmd320_block(istate, M);
    } else {
        rmd320_copy_bytes(M, 0, pass, (plen < 64) ? plen : 64);
        if (plen < 64) ((uchar *)M)[plen] = 0x80;
        rmd320_block(istate, M);
        for (int i = 0; i < 16; i++) M[i] = 0;
        int rem = plen - 64;
        if (rem > 0) rmd320_copy_bytes(M, 0, pass + 64, rem);
        if (plen >= 64) ((uchar *)M)[(rem > 0) ? rem : 0] = 0x80;
        M[14] = (64 + plen) * 8;
        rmd320_block(istate, M);
    }
    ...
    uint ostate[10] = { 0x67452301u, 0xEFCDAB89u, 0x98BADCFEu, 0x10325476u, 0xC3D2E1F0u,
                         0x76543210u, 0xFEDCBA98u, 0x89ABCDEFu, 0x01234567u, 0x3C2D1E0Fu };
...
__kernel void hmac_rmd320_kpass_batch(
    __global const uchar *hexhashes, __global const ushort *hexlens,
    __global const uchar *salts, __global const uint *salt_offsets, __global const ushort *salt_lens,
    __global const uint *compact_fp, __global const uint *compact_idx,
    __global const OCLParams *params_buf,
    ...
    __global const uint *overflow_offsets, __global const ushort *overflow_lengths)
{
    ...
    for (int i = 0; i < 16; i++) M[i] = 0;
    if (mlen <= 55) {
        rmd320_copy_bytes(M, 0, msg, mlen);
        ((uchar *)M)[mlen] = 0x80;
        M[14] = (64 + mlen) * 8;
        rmd320_block(istate, M);
    } else {
        rmd320_copy_bytes(M, 0, msg, (mlen < 64) ? mlen : 64);
        if (mlen < 64) ((uchar *)M)[mlen] = 0x80;
        rmd320_block(istate, M);
        for (int i = 0; i < 16; i++) M[i] = 0;
        int rem = mlen - 64;
        if (rem > 0) rmd320_copy_bytes(M, 0, msg + 64, rem);
        if (mlen >= 64) ((uchar *)M)[(rem > 0) ? rem : 0] = 0x80;
        M[14] = (64 + mlen) * 8;
        rmd320_block(istate, M);
    }
