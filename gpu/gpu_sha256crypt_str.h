...
static const char gpu_sha256crypt_str[] =
    ...
    " */\n"
    "\n"
    "/* ---- SHA256 primitives (big-endian internal) ---- */\n"
    "\n"
    "uint bswap32_sc(uint x) {\n"
    "    return ((x >> 24) & 0xffu) | ((x >> 8) & 0xff00u) |\n"
    "           ((x << 8) & 0xff0000u) | ((x << 24) & 0xff000000u);\n"
    "}\n"
    "\n"
    "__constant uint K256_SC[64] = {\n"
    ...
    "\n"
    "uint sc_rotr32(uint x, uint n) { return (x >> n) | (x << (32 - n)); }\n"
    "\n"
    "void sc_sha256_block(uint *state, uint *M) {\n"
    "    uint W[64];\n"
    "    for (int i = 0; i < 16; i++) W[i] = M[i];\n"
    "    for (int i = 16; i < 64; i++) {\n"
    "        uint s0 = sc_rotr32(W[i-15], 7) ^ sc_rotr32(W[i-15], 18) ^ (W[i-15] >> 3);\n"
    "        uint s1 = sc_rotr32(W[i-2], 17) ^ sc_rotr32(W[i-2], 19) ^ (W[i-2] >> 10);\n"
    "        W[i] = W[i-16] + s0 + W[i-7] + s1;\n"
    "    }\n"
    "    uint a = state[0], b = state[1], c = state[2], d = state[3];\n"
    "    uint e = state[4], f = state[5], g = state[6], h = state[7];\n"
    "    for (int i = 0; i < 64; i++) {\n"
    "        uint S1 = sc_rotr32(e, 6) ^ sc_rotr32(e, 11) ^ sc_rotr32(e, 25);\n"
    "        uint ch = (e & f) ^ (~e & g);\n"
    "        uint t1 = h + S1 + ch + K256_SC[i] + W[i];\n"
    "        uint S0 = sc_rotr32(a, 2) ^ sc_rotr32(a, 13) ^ sc_rotr32(a, 22);\n"
